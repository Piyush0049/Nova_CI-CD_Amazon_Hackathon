...
export function generateRuntimeLaunchCommands(config: RuntimeConfig): string[] {
  ...
  commands.push(
    ...
    `  echo ""`,
    `  echo "[RUNTIME] ════════════════════════════════════════════════════════════"`,
    `  echo "[RUNTIME] ❌ DEPLOYMENT FAILED: Application is not responding to HTTP requests"`,
    `  echo "[RUNTIME] ════════════════════════════════════════════════════════════"`,
    `  echo "[RUNTIME] Common causes:"`,
    `  echo "[RUNTIME]   - Application crashed during startup"`,
    `  echo "[RUNTIME]   - App is not binding to 0.0.0.0 (required for external access)"`,
    `  echo "[RUNTIME]   - App is not using PORT environment variable (PORT=${config.port})"`,
    `  echo "[RUNTIME]   - App is listening on a different port than detected"`,
    `  echo "[RUNTIME]   - Application has errors preventing it from starting"`,
    `  echo ""`,
    `  echo "[RUNTIME] 💡 To fix:"`,
    `  echo "[RUNTIME]   1. Check application logs above for errors"`,
    `  echo "[RUNTIME]   2. Ensure your app uses: process.env.PORT || ${config.port}"`,
    `  echo "[RUNTIME]   3. Ensure your app binds to: process.env.HOST || '0.0.0.0'"`,
    `  echo "[RUNTIME]   4. Test locally: PORT=${config.port} HOST=0.0.0.0 npm start"`,
    `  exit 1`,
    'fi',
  );
