...
export async function POST(request: NextRequest) {
  try {
    ...
    // Re-analyze repository to get port and startCommand if missing or outdated
    // This ensures we always use the current codebase's port, even if the pipeline was created before this feature was added
    if (!savedPipeline.port || !savedPipeline.startCommand) {
      ...
      try {
        ...
        console.log('[SMART-DEPLOY] Analyzing project with Amazon Nova AI...');
        const analysis = await analyzeUniversalProject(projectFiles);

        console.log('[SMART-DEPLOY] ✅ Re-analysis complete:');
        console.log('[SMART-DEPLOY]   - Detected Port:', analysis.port);
        console.log('[SMART-DEPLOY]   - Detected Start Command:', analysis.startCommand);

        // Update the saved pipeline with the detected values
        savedPipeline.port = analysis.port || '3000';
        savedPipeline.startCommand = analysis.startCommand || 'npm start';
        ...
        console.log('[SMART-DEPLOY] ✅ Updated pipeline with detected port and startCommand');
      } catch (reAnalysisError: any) {
        console.error('[SMART-DEPLOY] ⚠️ Re-analysis failed:', reAnalysisError.message);
        console.log('[SMART-DEPLOY] Falling back to defaults: port=3000, startCommand=npm start');
    ...
    const runInstancesCommand = new RunInstancesCommand({
      ...
      UserData: Buffer.from(`#!/bin/bash
        ...
        # Install Node.js 20.x (required for Next.js 16 and modern frameworks)
...
chmod 600 .env`
    ...
    console.log('[SMART-DEPLOY] Running pre-flight checks and fixes...');
    const preFlightResult = await executeSSMCommand(instanceId, [
      ...
      'if grep -q "react-scripts\\|\\"vite\\"\\|@vitejs" package.json 2>/dev/null; then',
      ...
      'fi',
      '',
      '# Delete package-lock.json if we modified package.json',
      'if [ "$NEEDS_PACKAGE_LOCK_REFRESH" = true ]; then',
      '  echo "  → Package.json updated - removing package-lock.json"',
      '  echo "  → npm install will regenerate with all new dependencies"',
      '  rm -f package-lock.json',
      '  echo "  ✅ Ready for fresh install with complete dependencies"',
      'fi',
      '',
...
async function runStagesWithAutoFix(
  ...
  for (let i = 0; i < pipeline.stages.length; i++) {
    ...
    if (!useAICommand) {
      for (const job of stageJobs) {
        for (const cmd of job.script) {
          ...
          } else if (stageName === 'build' && (cmd.includes('npm run build') || cmd.includes('react-scripts build') || cmd.includes('next build') || cmd.includes('vite build'))) {
          // UNIVERSAL BUILD DETECTION - works for ALL build commands
          stageCommands.push(`echo ""`);
          stageCommands.push(`echo "========================================================"`);
          stageCommands.push(`echo "[BUILD] 🔍 DETECTING PROJECT TYPE"`);
          stageCommands.push(`echo "========================================================"`);
          stageCommands.push(``);
          stageCommands.push(`# Check project type (frontend vs backend)`);
          stageCommands.push(`IS_FRONTEND=0`);
          stageCommands.push(`IS_BACKEND=0`);
          stageCommands.push(`# 1. Check for explicit frontend config files`);
          stageCommands.push(`if [ -f "vite.config.js" ] || [ -f "vite.config.ts" ] || [ -f "vite.config.cjs" ]; then`);
          stageCommands.push(`  echo "[BUILD] ✓ Vite config found - FRONTEND PROJECT"`);
          stageCommands.push(`  IS_FRONTEND=1`);
          stageCommands.push(`elif [ -f "webpack.config.js" ]; then`);
          stageCommands.push(`  echo "[BUILD] ✓ Webpack config found - FRONTEND PROJECT"`);
          stageCommands.push(`  IS_FRONTEND=1`);
          stageCommands.push(`elif [ -f "next.config.js" ] || [ -f "next.config.mjs" ]; then`);
          stageCommands.push(`  echo "[BUILD] ✓ Next.js config found - FRONTEND PROJECT"`);
          stageCommands.push(`  IS_FRONTEND=1`);
          stageCommands.push(`# 2. Check for explicit backend indicators`);
          stageCommands.push(`elif [ -f "index.js" ] || [ -f "server.js" ] || [ -f "app.js" ]; then`);
          stageCommands.push(`  if grep -q "\\"express\\"\\|"fastify"\\|"koa"\\|"@nestjs"" package.json 2>/dev/null; then`);
          stageCommands.push(`    echo "[BUILD] ✓ Backend server files + backend framework - BACKEND PROJECT"`);
          stageCommands.push(`    IS_BACKEND=1`);
          stageCommands.push(`  fi`);
          stageCommands.push(`# 3. Only as fallback, check package.json`);
          stageCommands.push(`elif grep -q "\\"react\\"\\|"@angular/core"\\|"vue"" package.json 2>/dev/null; then`);
          stageCommands.push(`  echo "[BUILD] ✓ Frontend framework in package.json - FRONTEND PROJECT"`);
          stageCommands.push(`  IS_FRONTEND=1`);
          stageCommands.push(`fi`);
          stageCommands.push(``);
          stageCommands.push(`# Check if build script exists`);
          stageCommands.push(`HAS_BUILD_SCRIPT=0`);
          stageCommands.push(`if grep -q "\\"build\\".*:.*\\"" package.json 2>/dev/null; then`);
          stageCommands.push(`  echo "[BUILD] ✓ Build script found in package.json"`);
          stageCommands.push(`  HAS_BUILD_SCRIPT=1`);
          stageCommands.push(`else`);
          stageCommands.push(`  echo "[BUILD] ⚠ No build script in package.json"`);
          stageCommands.push(`fi`);
          stageCommands.push(``);
          stageCommands.push(`echo "========================================================"`);
          stageCommands.push(`# Skip build for pure backends, run build for frontends or if explicit build script`);
          stageCommands.push(`if [ "$IS_BACKEND" = "1" ]; then`);
          stageCommands.push(`  echo "[BUILD] ⏭️  SKIPPING BUILD (Backend project detected)"`);
          stageCommands.push(`  echo "========================================================"`);
          stageCommands.push(`  echo "No build needed - backend project" > .build-skipped`);
          stageCommands.push(`elif [ "$IS_FRONTEND" = "1" ] || [ "$HAS_BUILD_SCRIPT" = "1" ]; then`);
          stageCommands.push(`  echo "[BUILD] 🏗️  RUNNING BUILD (Frontend or has build script)"`);
          stageCommands.push(`  echo "========================================================"`);
          stageCommands.push(``);

          // USE AI-GENERATED BUILD CONFIG (if available)
          if (buildConfig && buildConfig.framework !== 'unknown') {
            stageCommands.push(`  # ⚡ AI-OPTIMIZED BUILD for ${buildConfig.framework}`);
            stageCommands.push(`  echo "[BUILD] ⚡ Using AI-generated config for ${buildConfig.framework}..."`);
            stageCommands.push(`  echo "[BUILD] → Estimated time: ${buildConfig.estimatedBuildTime}"`);

            // Apply AI-generated environment variables
            Object.entries(buildConfig.environmentVars).forEach(([key, value]) => {
              stageCommands.push(`  export ${key}="${value}"`);
            });

            // Apply AI-generated optimization flags
            buildConfig.optimizationFlags.forEach(flag => {
              stageCommands.push(`  export ${flag}`);
            });

            stageCommands.push(`  echo "[BUILD] ✓ AI optimizations applied"`);
            stageCommands.push(``);

            // Use AI-generated build command if different from pipeline
            const aiBuildCommand = buildConfig.buildCommand;
            const finalCmd = aiBuildCommand !== 'npm run build' ? aiBuildCommand : cmd;

            // Progress monitoring based on AI recommendation
            if (buildConfig.progressMonitoring) {
              stageCommands.push(`  # Run build with AI-powered progress monitoring`);
              stageCommands.push(`  echo "[BUILD] 🏗️  Starting ${buildConfig.framework} build..."`);
              stageCommands.push(`  START=$(date +%s)`);
              stageCommands.push(`  ${finalCmd} &`);
              stageCommands.push(`  PID=$!`);
              stageCommands.push(`  while kill -0 $PID 2>/dev/null; do`);
              stageCommands.push(`    ELAPSED=$(($(date +%s) - START))`);
              stageCommands.push('    echo "[BUILD] ⏳ ${ELAPSED}s elapsed (target: ' + buildConfig.estimatedBuildTime + ')..."');
              stageCommands.push(`    sleep 15`);
              stageCommands.push(`  done`);
              stageCommands.push(`  wait $PID`);
              stageCommands.push(`  EXIT=$?`);
              stageCommands.push(`  TOTAL=$(($(date +%s) - START))`);
              stageCommands.push('  [ $EXIT -eq 0 ] && echo "[BUILD] ✅ Complete in ${TOTAL}s" || (echo "[BUILD] ❌ Failed" && exit $EXIT)');
            } else {
              // Fast build without monitoring
              stageCommands.push(`  echo "[BUILD] 🏗️  Running ${buildConfig.framework} build..."`);
              stageCommands.push(`  ${finalCmd}`);
            }
          } else {
            // FALLBACK: Use generic optimizations if AI config unavailable
            stageCommands.push(`  # ⚡ GENERIC BUILD OPTIMIZATIONS`);
            stageCommands.push(`  echo "[BUILD] ⚠ Using fallback config (AI unavailable)"`);
            stageCommands.push(`  export CI=true`);
            stageCommands.push(`  export NODE_OPTIONS="--max-old-space-size=6144"`);
            stageCommands.push(`  echo "[BUILD] ✓ Basic optimizations applied"`);
            stageCommands.push(``);
            stageCommands.push(`  echo "[BUILD] 🏗️  Starting build..."`);
            stageCommands.push(`  START=$(date +%s)`);
            stageCommands.push(`  ${cmd} &`);
            stageCommands.push(`  PID=$!`);
            stageCommands.push(`  while kill -0 $PID 2>/dev/null; do`);
            stageCommands.push(`    ELAPSED=$(($(date +%s) - START))`);
            stageCommands.push('    echo "[BUILD] ⏳ ${ELAPSED}s elapsed..."');
            stageCommands.push(`    sleep 15`);
            stageCommands.push(`  done`);
            stageCommands.push(`  wait $PID`);
            stageCommands.push(`  EXIT=$?`);
          ...
          stageCommands.push(`else`);
          stageCommands.push(`  echo "[BUILD] ⏭️  SKIPPING BUILD (No build configuration detected)"`);
          stageCommands.push(`  echo "========================================================"`);
          stageCommands.push(`  echo "No build needed" > .build-skipped`);
          stageCommands.push(`fi`);
          } else {
            stageCommands.push(cmd);
          }
    ...
    while (attempts < 2) {
      ...
      const result = await executeSSMCommand(instanceId, commands, timeout);
      ...
      if (attempts === 1) {
        ...
        if (fixResult.success && fixResult.fixCommands && fixResult.fixCommands.length > 0) {
          ...
          const combinedFixCommands = [
            ...envSetupCommands,
            'echo "==================================="',
            'echo "[AI-FIX] APPLYING NOVA AI FIX"',
            'echo "==================================="',
    ...
    for (let i = 0; i < 180; i++) {
      ...
      if (result.Status === 'Success') {
        ...
        return {
          success: false,
          output: fullOutput,
          error: fullError || recentOutput || 'Command failed',
        };
      }
    }
